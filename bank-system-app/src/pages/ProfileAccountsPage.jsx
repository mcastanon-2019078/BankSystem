...
import { TableAccountProfile } from '../components/Tables/AccountProfile'
...
export const ProfileAccountsPage = () => {
  ...
  const updateData = async () => {
    try {
      getTableAccountProfile();
    } catch (e) {
      console.log(e);
    }
  }

  const handleOpenModalChange = (balances) => {
    let datos1 = {
      balances: balances
    }
    setDatos(datos1)
  ...
  return (
    <>
        ...
        tableProfileAccount.map(({ _id, user, balances, state, typeAccount }, index) => {
          return (
            <div key={index}>
              ...
              ></TableAccountProfile>
              <svg onClick={() => handleOpenModalChange(balances)} xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-currency-exchange" viewBox="0 0 16 16" style={{ width: '100%', transform: 'translate(25%, -450%)', position: 'absolute' }}>
      ...
      {showModalTransfer && <ModalTransder isOpen={showModalTransfer} onClose={handleCloseModal} update={updateData} />}
      <ModalChange isOpen={showModalChange} onClose={handleCloseModalChange} datos={datos} />
    </>
  )
}
